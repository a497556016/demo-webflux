 ...
 */
package com.heshw.game.demowebflux.utils.excel;

import com.google.common.collect.Lists;
import com.heshw.game.demowebflux.utils.Reflections;
...
public class ImportExcel {
			...
			for (Object[] os : annotationList){
				...
				if (val != null){
					...
					/*if (StringUtils.isNotBlank(ef.dictType())){
						...
						val = dictService.selectByType(ef.dictType()).getValue(val.toString());
	 ...
	 * @param groups 导入分组
	 */
	public <E> List<E> getDataListViaHeader(Class<E> cls, int... groups) throws InstantiationException, IllegalAccessException {
		List<Object[]> annotationList = Lists.newArrayList();
		// Get annotation field
		Field[] fs = cls.getDeclaredFields();
		for (Field f : fs){
		...
		// Get excel data
		List<E> dataList = Lists.newArrayList();
		Row headerRow = this.getRow(headerNum);
		int lastRowNum = this.sheet.getLastRowNum();
		for (int i = this.getDataRowNum(); i <= lastRowNum; i++) {
			Row row = this.getRow(i);
			E e = (E)cls.newInstance();
			for(int j = 0;j < headerRow.getLastCellNum();j++) {
				Object val = this.getCellValue(row, j);
				if (val != null){
					String header = this.getCellValue(headerRow, j).toString();
					if(header.startsWith("*")) header = header.substring(1);
					for (Object[] os : annotationList){
						ExcelField ef = (ExcelField)os[0];
						if(!header.equals(ef.title())) continue;
						// If is dict type, get dict value
						if (StringUtils.isNotBlank(ef.dictType())){
//							val = DictUtils.getDictValue(val.toString(), ef.dictType(), "");
						...
						try {
							...
							log.info("Get cell value ["+i+","+j+"] error: " + ex.toString());
							val = null;
						}
						// set entity value
						if (os[1] instanceof Field){
	...
	public static void main(String[] args) throws Throwable {
		...
		ImportExcel ei = new ImportExcel("F://export.xlsx", 1);
